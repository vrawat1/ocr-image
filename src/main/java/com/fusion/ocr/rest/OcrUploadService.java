...
import com.fusion.domain.UploadedFile;
...
public class OcrUploadService {
	...
	@Path("/upload")
	public Response uploadResumeImage(ImageDocument document, @Context UriInfo uriInfo) {
		System.out.println("Title :" +document.getTitle());
		//System.out.println("File Contents :" +document.getFileContents());
		UploadedFile uploadedFile = new UploadedFile();
		if (StringUtils.hasLength(document.getFileContents())) {
			File savedFile = ocrService.saveImage(document);
			String extractedText = ocrService.processImage(savedFile.getName());
			uploadedFile = new UploadedFile(savedFile.getName(),
					Long.valueOf(savedFile.length()).intValue(),
					"/images/"+savedFile.getName());
			uploadedFile.setTextContents(extractedText);
			if (extractedText != null) {
				uploadedFile.setPersonalInfo(ocrService.parseBusinessCard(extractedText));
			}	
		}
		return Response.status(Response.Status.CREATED.getStatusCode())
		          .header(
		            "Location", 
		            String.format("%s",uriInfo.getAbsolutePath().toString()))		
		          .entity(uploadedFile).build();
	}
	...
	public Response downloadImage(@PathParam("name") String name) {
		...
		document.setTitle("bas.jpg");
		document.setFileContents("ABCDEFGH0123456789");
